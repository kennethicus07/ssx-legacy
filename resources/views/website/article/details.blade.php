...
@section('content')
    <div class="section carousel-holder full-vh" id="home-main">
        <div id="heroCarousel" class="carousel slide carousel-fade hero-carousel" data-bs-ride="carousel">
            <div class="carousel-inner">
                <div class="carousel-item active">
                    <img src="{{ url('storage/articles/banners/' . $article->image_banner) }}" class="d-block w-100"
                        alt="...">
                </div>
            </div>
        </div>
        ...
        <div class="section excerpt-info">
            <div class="content">
                <p class="date text-uppercase">POSTED
                    {{ \Carbon\Carbon::parse($article->created_at)->format('M d, Y - h:i A') }}</p>
                <h1>{{ $article->title }}</h1>
        ...
        <div class="section ssx-info beige-bg resource-holder">
            <div class="content black">
                            ...
                            <div class="card-body">
                                ...
                                <p class="card-text simple-lnk">
                                    ...
                                    <a href="{{ route('news-articles.details', [$latest_article->slug]) }}"
                                        class="arrow_btn">Learn More</a>
                                </p>
                                @if (!empty($latest_article->category_tag))
                                    <p class="tags">
                                        @foreach ($latest_article->category_tag as $tag)
                                            @if ($tag->sub_category)
                                                <span class="badge rounded-pill lightgreen-bg">
                                                    {{ $tag->sub_category->name }}
                                                </span>
                                            @endif
                                        @endforeach
                                    </p>
                ...
                <p class="align-right simple-lnk">
                    <a href="{{ route('news-articles.index') }}" class="arrow_btn">Read All Articles</a>
                </p>
            </div>
        </div>
    @endsection
